...
s = results["summary"]
reflection_content = f"""# Day 14 — Reflection
...
**Overall pass rate:** {s["pass_rate"]*100:.1f}%

| Metric | Average | Min | Max | Nhận xét |
|---|---:|---:|---:|---|
| Context Recall | {s["avg_context_recall"]:.3f} | 0.100 | 1.000 | Ở mức trung bình. BM25 bỏ sót nhiều tài liệu chứa thông tin đúng vì không nhận dạng được từ đồng nghĩa. |
| Context Precision | {s["avg_context_precision"]:.3f} | 0.583 | 1.000 | Rất cao. Các chunk liên quan (nếu tìm thấy) thường nằm ngay top đầu nhờ hàm rerank. |
| Faithfulness | {s["avg_faithfulness"]:.3f} | 0.000 | 1.000 | Thấp. Model thường xuyên bịa câu trả lời hoặc suy diễn ngoài tài liệu. |
| Relevance | {s["avg_relevance"]:.3f} | 0.000 | 1.000 | Thấp. Câu trả lời thường lạc đề do thiếu context. |
| Completeness | {s["avg_completeness"]:.3f} | 0.000 | 0.800 | Rất thấp. Các câu trả lời thường bị cắt cụt hoặc không đủ ý. |
| Overall Score | {(s["avg_context_recall"] + s["avg_context_precision"] + s["avg_faithfulness"] + s["avg_relevance"] + s["avg_completeness"]) / 5:.3f} | 0.000 | 0.859 | Tổng thể hệ thống cần tinh chỉnh mạnh mẽ. |
...
|---|---:|---:|
| hallucination | {s["failure_types"].get("hallucination", 0)} | {s["failure_types"].get("hallucination", 0)/20*100:.0f}% |
| irrelevant | {s["failure_types"].get("irrelevant", 0)} | {s["failure_types"].get("irrelevant", 0)/20*100:.0f}% |
| incomplete | {s["failure_types"].get("incomplete", 0)} | {s["failure_types"].get("incomplete", 0)/20*100:.0f}% |
| off_topic | {s["failure_types"].get("off_topic", 0)} | {s["failure_types"].get("off_topic", 0)/20*100:.0f}% |
| refusal | {s["failure_types"].get("refusal", 0)} | {s["failure_types"].get("refusal", 0)/20*100:.0f}% |
