...
class RAGASEvaluator:
    ...
    def evaluate_context_precision(
        ...
        ap_sum = sum((k + 1) / rank for k, rank in enumerate(relevant_chunks))
        return ap_sum / len(relevant_chunks)
...
class LLMJudge:
    ...
    def score_response(
        ...
        try:
            ...
            if isinstance(parsed, dict) and "scores" in parsed and "reasoning" in parsed:
                return parsed
        except Exception:
            pass
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            if "scores" in item:
                total_scores.extend(item["scores"].values())
...
class FailureAnalyzer:
    ...
    def find_root_cause(self, failure: EvalResult) -> str:
        ...
        elif failure.relevance <= failure.faithfulness and failure.relevance <= failure.completeness:
